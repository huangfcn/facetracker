...
void * pthread_mtcnn_run(void *)
{
    ...
    while (!bQuit)
    {
        ...
        sem_wait(&mtx_imgs_cap);
        Mat * pimg = imgs_capd[cap_rptr];
        cap_rptr = (cap_rptr + 1) & 63;
        sem_post(&mtx_imgs_cap);

        bbox_chain_t * pbbox = bbox_alloc();
        found.findFace(pimg->data, pbbox);

        sem_wait(&mtx_imgs_prc);
        imgs_proc[prc_wptr] = pimg;
        bbox_proc[prc_wptr] = pbbox;
...
void * pthread_mtcnn_trkn(void *)
{
    ...
    tracker_info_t tracker_info[MAX_FACES_PER_FRAME];

    double   distance[MAX_FACES_PER_FRAME * MAX_FACES_PER_FRAME];
    double * pdist;
    int      assigned[MAX_FACES_PER_FRAME];
    double   cost;

    int      assigned_trackers[MAX_FACES_PER_FRAME];
    int      assigned_detected[MAX_FACES_PER_FRAME];
    int      losted_trackers  [MAX_FACES_PER_FRAME];
...
{
    sem_init(&sem_imgs_spc, 0,  MTCNN_THREADS);
